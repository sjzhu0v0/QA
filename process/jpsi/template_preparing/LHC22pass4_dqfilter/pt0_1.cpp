...
void pt0_1() {
  ...
  gPublisherCanvas = new MPublisherCanvas("./mass.pdf", 1, 1, 600, 600);
  int i_pt = 1;
  ...
  TF1 *f1 = new TF1("f1", lFumula, 1.5, 5., 2);
  f1->SetParameters(1074.42, 6.37887);
  h_delta3->Fit(f1, "", "same", 1.5, 5.);
  ...
  // // --- 背景部分：自定义函数 ---
  // RooRealVar p0("p0", "Background amplitude", 56810, 10, 10000);
  // RooRealVar p1("p1", "Background exponent", 6, 4, 10);
  // // RooRealVar p2("p2", "Background numerator", 2, 0, 4);
  // RooGenericPdf bkgPDF("bkgPDF", "Background PDF",
  //                      "p0 * (x - 1.5)^2 / (1 + (x-1.5)^p1)",
  //                      RooArgList(x, p0, p1));
  // RooRealVar nsig("nsig", "Number of signal events", 3.7184e+04, 1e3, 2e5);
  // RooRealVar nbkg("nbkg", "Number of background events", 1.4669e+04, 1e3,
  // 1e5); RooAddPdf model("model", "Total PDF", RooArgList(cb, bkgPDF),
  ...
  for (int j = 1; j <= hist_copy->GetNbinsX(); ++j) {
    ...
    double crystalball_value =
        crystalball->Eval(hist_copy->GetBinCenter(j)) * 0.8;
