...
void pt2_3_final() {
  ...
  gPublisherCanvas = new MPublisherCanvas("./pt2_3_final.pdf", 1, 1, 600, 600);
  ...
  TF1 *bkg = new TF1("bkg",
                     "1./(exp([bkg0]*pow(x,[bkg1])) + "
                     "exp([bkg2]+[bkg3]/pow(x-[bkg4],[bkg5])))",
                     1.5, 5.);

  //  3  p2           3.65610e-01   7.36285e-03  -1.30210e-04  -1.31903e+01
  //  4  p3           9.27932e+01   1.10475e+00   1.71644e-02   7.65965e-01
  //  5  p4          -7.91297e+00   1.81180e-01  -2.82322e-03   9.03942e+00
  //  6  p5          -2.97035e+01   2.12246e+00   3.30106e-02   1.63413e+00
  //  7  p6          -7.16291e-01   1.10499e-02  -1.72125e-04   2.46565e+02
  //  8  p7           1.33651e+00   6.16986e-03   1.08470e-04  -1.79394e+01

#define parfast                                                                \
  3.65610e-01, 1.33651e+00, 9.27932e+01, -7.91297e+00, -2.97035e+01,           \
      -7.16291e-01
  ...
  auto lFormulaTotal3 = [signal, bkg](double *x, double *par) {
    ...
    // double bkg4 = par[6];
    return par[0] * signal->Eval(x1) +
           par[1] * (x1 - bkg2) * (x1 - bkg2) / (exp(bkg0 * x1) + bkg1);
  };
  gPublisherCanvas->SetCanvasNwNh(3, 1);
  for (int i_mult = 1; i_mult <= 5; i_mult++) {
    auto h1_lowMult = hn_tool.Project(1, {0, 3, i_mult});
    TF1 *f1_total =
    ...
    TF1 *f1_total_3 =
        new TF1(Form("f1_total_3_%d", i_mult), lFormulaTotal3, 2., 5., 4);
    f1_total_3->SetParameters(n_sig, n_bkg);
    f1_total_3->FixParameter(0, n_sig);
    h1_lowMult->Fit(f1_total_3, "0M", "same", 2., 5.);
  ...
  TFile *file = new TFile("/home/szhu/work/alice/analysis/QA/output/jpsi/"
                          "fit_template/LHC22pass4_dqfilter/pt2_3.root",
                          "RECREATE");
  file->cd();
  signal->Write();
  bkg->Write();
