...
void ptHigh() {
  ...
  gPublisherCanvas = new MPublisherCanvas("./mass_ptHigh.pdf", 1, 1, 600, 600);
  ...
  TF1 *bkg = new TF1("bkg", "1./(exp([bkg0]*x) + exp([bkg1]))", 1.2, 5.);
...
#define parfast 1.0972e+00, 5.2326
  ...
  // TF1 *bkg_qa = new TF1(
  //     "bkg_qa", "exp([bkg0]*x)
  //     +exp([bkg1]+[bkg2]/pow(x-[bkg3],[bkg4]))", 1.2, 5.);
  ...
  TFile *file = new TFile("/home/szhu/work/alice/analysis/QA/output/jpsi/"
                          "fit_template/LHC22pass4_dqfilter/ptHigh.root",
                          "RECREATE");
  file->cd();
  signal->Write();
  bkg->Write();
