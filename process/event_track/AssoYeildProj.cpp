...
public:
  ...
  void SetRangeUser(int dim, double min, double max) {
    if (dim < 0 || dim >= fHnSame->hN->GetNdimensions()) {
      cerr << "Error: AssocYieldHelperREF::SetRangeUser: dim is out of range"
           << endl;
      exit(1);
    }
    switch (dim) {
  ...
  TH2D *AssociatedYieldVtxZ(int iVtxZ, int iMult, bool doNTrigScale = true) {
    ...
    TH2D *h_assoYield =
        (TH2D *)h2D->Clone(Form("h_assoYield_%d_%d", iVtxZ, iMult));
  ...
  TH2D *AssociatedYieldVtxZPtSum(int iVtxZ, int iMult,
...
void AssoYieldProj(
  ...
  auto h2_sub_highLow_mass_pt =
      (TH2D *)h2_highMult_mass_pt->Clone("h2_sub_highLow_mass_pt");
  HistSubstraction2D(h2_sub_highLow_mass_pt, h2_highMult_mass_pt,
                     h2_lowMult_mass_pt);
...
int main(int argc, char **argv) {
  ...
  AssoYieldProj(path_input_se_pr, path_input_se_raw, path_input_me_pr,
                path_output);
