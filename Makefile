...
macro/event_jpsi/EventMixingJpsiAsso.exe: macro/event_jpsi/EventMixingJpsiAsso.cpp opt/libMRootDict.o opt/EventData.cxx
	g++ -o $@ $^ $(FLAGS_INCLUDE) $(FLAGS_ROOT) $(FLAGS_MINUIT) -I./

opt/MRootDict.cxx:  opt/EventData.h opt/LinkDef.h
	rootcint -f $@ -c $^ $(FLAGS_INCLUDE)

opt/libMRootDict.o: opt/MRootDict.cxx
	g++ -o $@ -c $^ -I./ `root-config --cflags --libs`