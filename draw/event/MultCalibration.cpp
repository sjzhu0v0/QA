...
void MultCalibration(
  ...
  auto fNumContribfPosZRun = (TProfile2D*)file_input->Get("fNumContribfPosZRun");

  TFile* file_output = new TFile(path_output, "RECREATE");

  MRootGraphic::StyleHistCommonHist(fNumContribfPosZ);

  TCanvas* c_fNumContribfPosZ = new TCanvas("c_fNumContribfPosZ", "c_fNumContribfPosZ", 600, 600);
  ...
  c_fNumContribfPosZ->SaveAs(path_plot + "MultCalibration_fNumContribfPosZ_" + tag_period + ".pdf");
  c_fNumContribfPosZ->SaveAs(path_plot + "MultCalibration_fNumContribfPosZ_" + tag_period +
                             ".json");

  TCanvas* c_fNumContribRun = new TCanvas("c_fNumContribRun", "c_fNumContribRun", 1200, 600);
  c_fNumContribRun->cd();
  ...
  c_fNumContribRun->SaveAs(path_plot + "MultCalibration_fNumContribRun_" + tag_period + ".pdf");
  c_fNumContribRun->SaveAs(path_plot + "MultCalibration_fNumContribRun_" + tag_period + ".json");
  ...
  fNumContribfPosZRun->SetTitle("fNumContribfPosZRun;fPosZ [cm];Run;<fNumContrib>");
  ...
  c_fNumContribfPosZRun->SaveAs(path_plot + "MultCalibration_fNumContribfPosZRun_" + tag_period +
                                ".pdf");
  c_fNumContribfPosZRun->SaveAs(path_plot + "MultCalibration_fNumContribfPosZRun_" + tag_period +
                                ".json");

  TCanvas* c_fNumContribfPosZRun_calib =
      new TCanvas("c_fNumContribfPosZRun_calib", "c_fNumContribfPosZRun_calib", 600, 600);
  ...
  for (int i_run = 1; i_run <= fNumContribfPosZRun->GetNbinsY(); i_run++) {
    ...
    for (int i_vtxz = 1; i_vtxz <= h_NumContribPosZ_calibration->GetNbinsX(); i_vtxz++) {
    ...
    file_output->WriteObject<TF1>(fit_func, Form("fNumContribfPosZRun_calib_%s", label.Data()));
    ...
    if (i_run == 1) {
      c_fNumContribfPosZRun_calib->SaveAs(path_plot + "MultCalibration_fNumContribfPosZRun_calib_" +
                                              tag_period + ".pdf(",
  ...
  TH1D* h_fNumContribfPosZ = fNumContribfPosZ->ProjectionX("h_NumContribPosZ_calibration");
  h_fNumContribfPosZ->GetYaxis()->SetTitle("1/(Normlized <fNumContrib>)");
  h_fNumContribfPosZ->Sumw2(false);
  h_fNumContribfPosZ->Scale(h_fNumContribfPosZ->GetNbinsX() / h_fNumContribfPosZ->Integral());
  ...
  c_h_fNumContribfPosZ->SaveAs(path_plot + "MultCalibration_fNumContribfPosZ_Calibration_" +
                               tag_period + ".pdf");
  c_h_fNumContribfPosZ->SaveAs(path_plot + "MultCalibration_fNumContribfPosZ_Calibration_" +
                               tag_period + ".json");
