...
void JpsiAsso(TString path_input_flowVecd = "../input.root",
  ...
  auto rdf_PartTriggerWithJpsiWithEvent =
      rdf_PartTrigger
          ...
          .Define("JpsiInfoUS",
                   ...
                   &up_edge_deltaPhiToPi](const EventData& eventData) {
                    ...
                    for (size_t i = 0; i < eventData.jpsi_info.fPT.size(); ++i) {
                      if (eventData.jpsi_info.fSign[i] == 0) {
                        for (size_t j = 0; j < eventData.track_info.fEtaREF.size(); ++j) {
                          if (!CutTrackInfo(eventData.track_info, j))
                            continue;
                          float delta_eta =
                              eventData.jpsi_info.fEta[i] - eventData.track_info.fEtaREF[j];
                          float delta_phi =
                              eventData.jpsi_info.fPhi[i] - eventData.track_info.fPhiREF[j];
                          int n = 0;
                  ...
                  {"EventData"})
          .Define("JpsiInfoUSSingle",
                  [](const EventData& eventData) {
                    ROOT::VecOps::RVec<array<float, 4>> vec2return;
                    for (size_t i = 0; i < eventData.jpsi_info.fPT.size(); ++i) {
                      if (eventData.jpsi_info.fSign[i] == 0) {
                        vec2return.push_back(
                            {eventData.event_info.fPosZ, eventData.event_info.fNumContribCalib,
                             eventData.jpsi_info.fMass[i], eventData.jpsi_info.fPT[i]});
                      }
                    }
                    return vec2return;
                  },
  ...
  StrVar4Hist var_MassJpsiCandidate("MassUS", "M_{ee}", "GeV^{2}/c^{4}", n_bins_mass_assoYield,
                                    {1.8, 5.4});
  ...
  StrVar4Hist var_NumContribCalibBinnedSingle("NumContribCalibUSSingle",
                                              "N_{vtx contrib} Calibrated", "", 10,
                                              {0, 7, 12, 17, 22, 29, 37, 46, 57, 73, 300});
  StrVar4Hist var_MassJpsiCandidateSingle("MassUSSingle", "M_{ee}", "GeV^{2}/c^{4}", 90,
                                          {1.8, 5.4});
  StrVar4Hist var_PtJpsiCandidateSingle("PtUSSingle", "p_{T}", "GeV/c", 10, {0., 5.});
...
#define obj2push_thnd(rdf2push, ...)                                                               \
  ...
  obj2push_thnd(rdf_PartTriggerWithJpsiWithEvent,
                {var_DeltaEtaUS, var_DeltaPhiUS, var_fPosZ, var_MassJpsiCandidate,
                 var_PtJpsiCandidate, var_NumContribCalibBinned});
  obj2push_thnd(rdf_PartTriggerWithJpsiWithEvent,
                {var_fPosZSingle, var_MassJpsiCandidateSingle, var_PtJpsiCandidateSingle,
                 var_NumContribCalibBinnedSingle});
