...
float JpsiRapidity(float pt, float eta, float mass) {
  const float transverse_mass = std::sqrt(mass * mass + pt * pt);
  return std::asinh(pt * std::sinh(eta) / transverse_mass);
}
...
vector<pair<ULong64_t, ULong64_t>> MixEvent(unsigned int, const int id, const ULong64_t& event_id) {
  return MixVec<pair<ULong64_t, ULong64_t>, ULong64_t>(
      id, event_id, [](const ULong64_t& a, const ULong64_t& b) { return make_pair(a, b); }, 20);
}
    ...
    for (const auto& [event_a, event_b] : *mixed_events) {
      ...
      for (int i_jpsi = 0; i_jpsi < fPT.GetSize(); ++i_jpsi) {
        ...
        for (int i_ref = 0; i_ref < fPTREF.GetSize(); ++i_ref) {
          ...
          const double pair_values[] = {JpsiRapidity(fPT[i_jpsi], fEta[i_jpsi], fMass[i_jpsi]) -
                                            fEtaREF[i_ref],
                                        DeltaPhi(fPhi[i_jpsi], fPhiREF[i_ref]),
                                        posz_value,
                                        fMass[i_jpsi],
                                        fPT[i_jpsi],
                                        mult_value};
