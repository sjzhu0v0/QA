...
RVec<float> DeltaPhiVec(const RVec<float>& phi1, const RVec<float>& phi2) {
  RVec<float> out;
  out.reserve(phi1.size());
  for (size_t i = 0; i < phi1.size(); ++i) {
    float delta = phi1[i] - phi2[i];
    while (delta >= 1.5f * M_PI)
      delta -= 2.f * M_PI;
    while (delta < -0.5f * M_PI)
      delta += 2.f * M_PI;
    out.push_back(delta);
  }
  return out;
...
RVec<float> JpsiRapidityVec(const RVec<float>& pt, const RVec<float>& eta,
                            const RVec<float>& mass) {
  RVec<float> out;
  out.reserve(pt.size());
  for (size_t i = 0; i < pt.size(); ++i) {
    const float transverse_mass = std::sqrt(mass[i] * mass[i] + pt[i] * pt[i]);
    out.push_back(std::asinh(pt[i] * std::sinh(eta[i]) / transverse_mass));
  }
  return out;
}
...
RVec<float> UnitWeightVec(const RVec<float>& pt) {
  return RVec<float>(pt.size(), 1.f);
}
...
void JpsiAssoRawPoiEff(TString path_input_flow,
  ...
  auto df_pairs =
      df_event
          ...
          .Define("jpsi_mass", [](const RVec<float>& v, const RVec<int>& i) { return Take(v, i); },
                  {"fMass", "jpsi_idx"})
          .Define("jpsi_rapidity", JpsiRapidityVec, {"jpsi_pt", "jpsi_eta", "jpsi_mass"})
          ...
          .Define("ref_dcaz", [](const RVec<float>& v, const RVec<int>& i) { return Take(v, i); },
                  {"fDcaZ", "ref_idx"})
          .Define("DeltaRapidity", "jpsi_rapidity - ref_eta")
          .Define("DeltaPhi", DeltaPhiVec, {"jpsi_phi", "ref_phi"})
  ...
  auto var_dy = ParseStrVar4Hist(config["hist_binning"]["DeltaRapidityUS"]);
  auto var_dphi = ParseStrVar4Hist(config["hist_binning"]["DeltaPhiUS"]);
  vector<StrVar4Hist> pair_vars = {var_dy, var_dphi, var_posz, var_mass, var_pt, var_mult};
  ...
  for (auto it = config["cuts"].begin(); it != config["cuts"].end(); ++it) {
    ...
    auto selected =
        df_pairs.Define(mask_name, cut_expr)
            .Redefine("DeltaRapidity", "DeltaRapidity[" + mask_name + "]")
...
int main(int argc, char** argv) {
  ...
  if (argc > 7)
    only_cut = argv[7];
  if (argc > 8)
    unit_efficiency = ParseBoolArg(argv[8]);
