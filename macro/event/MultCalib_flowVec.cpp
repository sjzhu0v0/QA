...
void MultCalib(TString path_input = "../input.root", TString path_input_ext = "../input_ext.root",
  ...
  gRResultHandlesFast.push_back(rdf_fullTrigger.Histo1D(
      {"fNumContrib", "fNumContrib;Counts;NumContrib", 300, 0, 300}, "fNumContrib"));
  gRResultHandlesFast.push_back(rdf_fullTrigger.Histo2D(
      {"fNumContrib_fMultTPC", "fNumContrib_fMultTPC;fNumContrib;fMultTPC;Counts", 300, 0, 300, 300,
       0, 300},
      "fNumContrib", "fMultTPC"));
  gRResultHandlesFast.push_back(rdf_fullTrigger.Histo2D(
      {"NumContribCalib_fMultTPC", "NumContribCalib_fMultTPC;NumContribCalib;fMultTPC;Counts", 300,
       0, 300, 300, 0, 300},
      "NumContribCalib", "fMultTPC"));
  gRResultHandlesFast.push_back(rdf_fullTrigger.Histo2D(
      {"fMultNTracksPV_fMultTPC", "fMultNTracksPV_fMultTPC;fMultNTracksPV;fMultTPC;Counts", 150, 0,
       150, 300, 0, 300},
      "fMultNTracksPV", "fMultTPC"));

  auto profile_fNumContribRun = rdf_fullTrigger.Profile1D(
      {"fNumContribRun", "fNumContribRun;run; fNumContrib", 1, 0., 1.}, "RunNumber", "fNumContrib");
  ...
  auto profile_NumContribCalibVtxZ = rdf_fullTrigger.Profile1D(
      {"NumContribCalibPosZ", "NumContribCalib;fPosZ [cm]; fNumContrib Calib", 10, -10, 10.},
  ...
  auto profile_fNumContribCalibRun = rdf_fullTrigger.Profile1D(
      {"NumContribCalibRun", "fNumContribCalibRun;run; fNumContrib Calib", 1, 0., 1.}, "RunNumber",
      "NumContribCalib");
  ...
  profile_fNumContribCalibRun->GetXaxis()->SetBinLabel(1, Form("%d", runNumber));
  fOutput->cd();
