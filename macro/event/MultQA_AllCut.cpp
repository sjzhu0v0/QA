...
void MultPileupCut(
    ...
    TString path_pileup = " /home/szhu/work/alice/analysis/QA/output/event/"
  ...
  auto rdf_witTrigger =
      rdf.Define("map_trigger", MALICE::triggermapRVec, {"fSelection"})
          ...
          .DefineSlot("NumContribCalib", Calib_NumContrib_fPosZ_Run::NumContribCalibratedFloat,
                      {"fNumContrib", "fPosZ"})
          .DefineSlot("isntSelfDefinedPileup", Cut_MultTPC_NumContrib::isInCutSlot,
  ...
  auto rdf_basicTrigger = rdf_witTrigger.Filter("isTriggerTVX", "is Trigger TVX")
                              .Filter("isntITSROFrameBorder", "no ITS RO Frame border")
                              .Filter("isntTimeFrameBorder", "no Time Frame border")
                              .Filter("isntSameBunchPileup", "no same bunch pileup");
  if (is_interactive())
  ...
  gRResultHandlesFast.push_back(rdf_witTrigger.Histo1D(
      {"LevelTrigger", "Trigger Level; Trigger Level", 6, -0.5, 5.5}, "LevelTrigger"));
  ...
  StrVar4Hist var_fNumContrib("fNumContrib", "fNumContrib", "a.u.", 300, {0, 300});
  StrVar4Hist var_fMultTPC("fMultTPC", "fMultTPC", "a.u.", 600, {0, 600});
  StrVar4Hist var_fMultFV0A("fMultFV0A", "fMultFV0A", "a.u.", 100, {0, 50.e3});
  StrVar4Hist var_fMultFT0A("fMultFT0A", "fMultFT0A", "a.u.", 310, {-1000., 30000.});
  StrVar4Hist var_fMultFT0C("fMultFT0C", "fMultFT0C", "a.u.", 130, {-1000., 12000.});
  StrVar4Hist var_fMultFDDA("fMultFDDA", "fMultFDDA", "a.u.", 360, {-1000, 35000});
  ...
  vector<StrVar4Hist> vec_var_mult = {var_fNumContrib, var_fMultFV0A, var_fMultFT0A,
...
#define localDefineHis1D(fVar, fTag)                                                               \
  ...
  for (auto& var_pos : vec_var_pos) {
    for (auto& var_mult : vec_var_mult) {
      gRResultHandlesFast.push_back(rdf_fullTrigger.Histo2D(
          GetTH2DModel(var_pos, var_mult, "fullTrigger"), var_pos.fName, var_mult.fName));
      gRResultHandlesFast.push_back(rdf_basicTrigger.Histo2D(
          GetTH2DModel(var_pos, var_mult, "basicTrigger"), var_pos.fName, var_mult.fName));
    }
  }
  ...
  for (int i_mult1 = 0; i_mult1 < vec_var_mult.size(); i_mult1++) {
    for (int i_mult2 = i_mult1 + 1; i_mult2 < vec_var_mult.size(); i_mult2++) {
      gRResultHandlesFast.push_back(rdf_fullTrigger.Histo2D(
          GetTH2DModel(vec_var_mult[i_mult1], vec_var_mult[i_mult2], "fullTrigger"),
          vec_var_mult[i_mult1].fName, vec_var_mult[i_mult2].fName));
      gRResultHandlesFast.push_back(rdf_basicTrigger.Histo2D(
          GetTH2DModel(vec_var_mult[i_mult1], vec_var_mult[i_mult2], "basicTrigger"),
          vec_var_mult[i_mult1].fName, vec_var_mult[i_mult2].fName));
    }
  }
